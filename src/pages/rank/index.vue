<template>
    <div class="rank">
        ...
        <div class="rank-list">
          <div class="rank-content" v-if="hasLoad">
            <h1 class="title">精选榜单</h1>

            <div class="top250 rank-item" @click="select('top250')">
              <div class="desc">
                <h2 class="name">豆瓣 Top250</h2>
                <span class="brief">8分以上好电影</span>
              </div>
              <div class="rank-img">
                <img v-for="(item,index) in urlList[0]" :key="index" :src="item" :class="{'top': index === 1 }" alt="">
              </div>
            </div>

            <div class="weekly rank-item" @click="select('weekly')">
              <div class="desc">
                <h2 class="name">本周口碑榜</h2>
                <span class="brief">{{weekDate}}</span>
              </div>
              <div class="rank-img">
                <img v-for="(item,index) in urlList[1]" :key="index" :src="item" :class="{'top': index === 1 }"/>
              </div>
            </div>

            <div class="new-movie rank-item" @click="select('newMovie')">
              <div class="desc">
                <h2 class="name">新片榜</h2>
                <span class="brief">{{weekDate}}</span>
              </div>
              <div class="rank-img">
                <img v-for="(item,index) in urlList[2]" :key="index" :src="item" :class="{'top': index === 1 }"/>
              </div>
            </div>

            <div class="us-box rank-item" @click="select('usBox')">
              <div class="desc">
                <h2 class="name">北美票房榜</h2>
                <span class="brief">票房最高排名</span>
              </div>
              <div class="rank-img">
                <img v-for="(item,index) in urlList[3]" :key="index" :src="item" :class="{'top': index === 1 }"/>
              </div>
            </div>
          </div>
        </div>
...
import WeekDate from '@/utils/date';
